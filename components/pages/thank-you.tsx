 ...
 *
 * Lleva `noindex` (lo pone la ruta) porque una pagina de gracias no le sirve
 * a nadie que llegue desde el buscador: no responde a ninguna busqueda y es
 * contenido minimo. La URL existe para quien la tenga guardada y para poder
 * enviar aqui el formulario el dia que interese medir la conversion con una
 * URL propia.
 */
...
export function ThankYouPage({ locale }: { locale: Locale }) {
  ...
  return (
    <>
      <SiteHeader locale={locale} path="/thank-you" />
      ...
      <main className={`${wrap} py-16 lg:py-24`}>
        <div className="mx-auto flex max-w-[52ch] flex-col items-center text-center">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            ...
            <CalendlyButton>{t.book}</CalendlyButton>
            <CtaLink href={localizePath("/", locale)} variant="ghost">
              {t.home}
            </CtaLink>
          </div>
      ...
      <SiteFooter locale={locale} path="/thank-you" />
    </>
  );
}
